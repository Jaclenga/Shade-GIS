...
- `shade_study_stops.csv`: active imported and prepared dataset that powers the public map, filters, metrics, and downloads. Imported custom columns are preserved.
- `shade_study_raw_labels.csv`: raw submitted labels, included when labels have been collected.
- `shade_study_config.json`: project metadata, methodology, editable terminology, shade taxonomy, visualization settings, uploaded GIS overlays, and import log.
...
shade-source checkboxes per pseudonymous visitor and stop. The app keeps the community result separate
...
Repeated-voting protections are enabled by default. The app stores only a server-keyed HMAC pseudonym,
not raw IP addresses or browser headers, and applies the cooldown and hourly limit configured in the
builder. These controls deter casual manipulation; high-stakes binding polls should additionally use
authentication, CAPTCHA, and external abuse monitoring.
...
SHADE_GIS_VOTE_FINGERPRINT_SECRET = "GENERATE_A_LONG_RANDOM_SECRET"
```

The vote tables are created automatically. `SHADE_GIS_VOTE_FINGERPRINT_SECRET` is optional because the
database can create and retain a random key, but setting it explicitly makes key rotation intentional.
Without `SHADE_GIS_VOTE_DATABASE_URL`, Streamlit Community Cloud uses the local SQLite fallback, whose
files are ephemeral and may be lost when the app restarts.
